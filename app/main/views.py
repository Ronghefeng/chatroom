...
def addper():
    form = CreatePerForm()
    old_per = Permission.query.filter_by(
        permission_name=form.permissionname.data
    ).first()
...
def chat_room_list():
    ...
    return render_template(
        "chatroomlist.html", roomlist=roomlist, can_create=can_create
    )
...
def create_room(chatwith=None):
    if chatwith:
        ...
        if r.exists("pchat-" + rname + "-" + current_user.username) is False:
            r.zadd(
                "pchat-" + rname + "-" + current_user.username, current_user.username, 1
            )
...
def chat():
    ...
    for i in messages:
        msg_list.append(
            [
                json.loads(i[0]),
                time.strftime("%Y/%m/%d %p%H:%M:%S", time.localtime(i[1])),
            ]
        )
    if current_user.is_authenticated:
        return render_template(
        ...
        gravatar_url = (
            "http://www.gravatar.com/avatar/" + hash + "?s=40&d=identicon&r=g"
        )
...
def private_chat():
    ...
    if user_right:
        ...
        messages = r.zrange(
            "pmsg-" + uname + "-" + current_user.username, 0, -1, withscores=True
        )
        msg_list = []
        for i in messages:
            msg_list.append(
        ...
        return render_template(
            "privatechat.html", rname=uname, user_list=ulist, msg_list=msg_list
        )
    else:
        pass
    ...
    if current_user.is_authenticated:
        ...
        if rtype and rtype == "p":
            ...
            if current_user.username in ulist:
                ...
                r.zadd(
                    "pmsg-" + rname + "-" + current_user.username,
                    json.dumps(body),
                    time.time(),
                )
...
def room_user_list():
    ...
    for b in b_user:
        b_user_list.append(r.get(b))
    return render_template(
        "roomuser_list.html", ulist=ulist, rname=rname, b_user=b_user_list
    )
